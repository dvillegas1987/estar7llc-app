...
class Product extends MyAppModel
{
    ...
    public static function getProductShippingRates($product_id, $lang_id, $country_id = 0, $sellerId = 0, $limit = 0)
    {
        ...
        $srch->addCondition('tpsr.' . static::DB_PRODUCT_TO_SHIP_PREFIX . 'prod_id', '=', 'mysql_func_' . intval($product_id), 'AND', true);
        if ($country_id > 0) {
    ...
    public static function getProductFreeShippingAvailabilty($product_id, $lang_id, $country_id = 0, $sellerId = 0)
    {
        ...
        $srch->addCondition('tps.' . static::DB_TBL_PRODUCT_SHIPPING_PREFIX . 'product_id', '=', 'mysql_func_' . intval($product_id), 'AND', true);
...
END,   special_price_found ) as special_price_found'
        ...
        if (array_key_exists('featured', $criteria)) {
            ...
            if (0 < $featured) {
                $srch->addCondition('product_featured', '=', 'mysql_func_' . $featured, 'AND', true);
            }
        }
    ...
    public static function getActiveCount($sellerId, $prodId = 0)
    {
        ...
        $srch->addCondition(static::DB_TBL_PREFIX . 'seller_id', '=', 'mysql_func_' . $sellerId, 'AND', true);
        ...
        $srch->addCondition(static::DB_TBL_PREFIX . 'deleted', '=', 'mysql_func_' . applicationConstants::NO, 'AND', true);
        $srch->addCondition(static::DB_TBL_PREFIX . 'approved', '=', 'mysql_func_' . applicationConstants::YES, 'AND', true);
        if ($prodId) {
            $srch->addCondition(static::DB_TBL_PREFIX . 'id', '!=', 'mysql_func_' . $prodId, 'AND', true);
        }
        $rs = $srch->getResultSet();
    ...
    public function getProdSpecificationsByLangId($langId)
    {
        ...
        $srch->addCondition(static::DB_PRODUCT_SPECIFICATION_PREFIX . 'product_id', '=', 'mysql_func_' . $this->mainTableRecordId, 'AND', true);
        $srch->addCondition(static::DB_PRODUCT_LANG_SPECIFICATION_PREFIX . 'lang_id', '=', 'mysql_func_' . $langId, 'AND', true);
        $srch->addMultipleFields(
            array(
                static::DB_PRODUCT_SPECIFICATION_PREFIX . 'id',
                static::DB_PRODUCT_SPECIFICATION_PREFIX . 'name',
    ...
    public static function getProductSpecificsDetails($productId)
    {
        ...
        $srch->addCondition(ProductSpecifics::DB_TBL_PREFIX . 'product_id', '=', 'mysql_func_' . $productId, 'AND', true);
        $rs = $srch->getResultSet();
        return FatApp::getDb()->fetch($rs);
    }
    ...
    public static function isShipFromConfigured($productId, $userId = 0)
    {
        ...
        $srch->addCondition('ps_product_id', '=', 'mysql_func_' . $productId, 'AND', true);
        $srch->addCondition('ps_user_id', '=', 'mysql_func_' . $userId, 'AND', true);
        $srch->setPageSize(1);
        $srch->doNotCalculateRecords();
